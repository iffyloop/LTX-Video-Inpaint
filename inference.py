...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="*",
        help="List of paths to conditioning masks (images or videos). Each path will be used as a conditioning mask.",
    )
    parser.add_argument(
    ...
    **kwargs,
    ...
    if conditioning_media_paths:
        ...
        if not conditioning_mask_paths:
            raise ValueError(
                "If `conditioning_media_paths` is provided, "
                "`conditioning_mask_paths` must also be provided"
            )
        if len(conditioning_media_paths) != len(conditioning_mask_paths) or len(
            conditioning_media_paths
        ) != len(conditioning_start_frames):
            raise ValueError(
                "`conditioning_media_paths`, `conditioning_start_frames`, "
                "`conditioning_mask_paths` must have the same length"
            )
...
def prepare_conditioning(
    ...
    """
    conditioning_items = []
    for path, mask_folder_path, start_frame in zip(
        ...
        if is_video:
            ...
            # These are in latent space dimensions, and 1 channel (gray) instead of 3 (rgb)
            mask_frames = []
            num_mask_frames = num_input_frames // 8 + 1
            for mask_frame_i in range(num_mask_frames):
                mask_frame_img = Image.open(
                    os.path.join(mask_folder_path, "{}.png".format(str(mask_frame_i)))
                ).convert("RGB")
                mask_frame_img = np.array(mask_frame_img).astype(np.float32) / 255.0
                mask_frame_img = mask_frame_img.mean(
                    axis=2, keepdims=True
                )  # RGB to Gray
                mask_frames.append(mask_frame_img)
            mask_frames = torch.tensor(np.array(mask_frames)).float()
            mask_frames = rearrange(mask_frames, "f h w c -> 1 c f h w")

            conditioning_items.append(
                ConditioningItem(video_tensor, mask_frames, start_frame)
            )

        else:  # Input image
            raise Exception(
                "Images not supported right now (please use a video and mask)"
            )
